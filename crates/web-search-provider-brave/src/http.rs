//! Provider-local HTTP helpers. Identical to the Tavily and SerpAPI
//! versions; we keep them per-crate so each provider can evolve its
//! own retry semantics if a backend grows quirky.
