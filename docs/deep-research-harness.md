...
All four v2 work items shipped:

- Six built-in topologies (three v1 + three v2:
  `PlanAndExecuteLoop`, `LinearWriteCritiqueLoop`,
  `OutlineFirstSectionFanoutLoop`).
- Provider crates for Tavily / SerpAPI / Brave that implement
  `WebSearch`.
- Two-tier outer shell (`atomr-agents-deep-research-shell`) with a
  pluggable `IntentClassifier` routing between a shallow path and the
  deep harness.
- `AgentBased{Role}` impls behind the `agent` feature flag, wrapping
  `atomr_agents_agent::Agent` for LLM-driven clarification, planning,
  research, drafting, critique, and verification — plus a domain
  `ResearchToolSet` exposing every `ResearchHandle` mutation as a
  `Tool`.
